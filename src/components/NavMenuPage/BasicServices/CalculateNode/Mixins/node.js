    ...
    methods: {
        getNodeData(removeLoading) {
            if (!removeLoading) {
                ...
                this.nodeManager.option.form[0]['loading'] = true
            }
            ...
            device.getNodeData()
                .then(res => {
                    ...
                    nodeList.forEach(item => {
                        ...
                        item.tableOption = {}
                        item.tableOption['defaultSort'] = { prop: 'serviceId', order: 'ascending' }
                    ...
                    this.nodeManager.option.form[0]['loading'] = false
        ...
        dealData() {
            this.physicalNode.forEach(item => {
                ...
                if (!item.status) {
                    item.leftButtons.buttons[3]['disabled'] = true
                    item.leftButtons.buttons[2]['disabled'] = true
                } else if (!item.tableOption.tableData.length) {
                    item.leftButtons.buttons[2]['disabled'] = true
                } else if (item.serviceStatus === 2) {
                    item.leftButtons.buttons[2]['disabled'] = true
                } else {
                    item.leftButtons.buttons[3]['disabled'] = false
                    item.leftButtons.buttons[2]['disabled'] = false
                }
            })
        ...
        restartNode() {
            this.$confirm('重启“物理”节点即重启物理机, 该操作将会中断当前所有任务, ' + '<br /> ' + ' 并重启硬件及服务', '重启节点', {
                ...
                this.physicalNode[this.activeTableIndex]['loading'] = true
                ...
                device.restartNode({ id: this.buttonScope.id })
                    .then(res => {
                        ...
                        this.physicalNode[this.activeTableIndex]['loading'] = false
                    })
                    .catch(() => {
                        this.physicalNode[this.activeTableIndex]['loading'] = false
                    })
