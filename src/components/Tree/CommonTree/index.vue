  ...
  >
    ...
    <div v-for="(dataItem,dataItemIndex) in dataGroup" :key="dataItemIndex" class="tree-block">
      ...
      <el-tree
        ...
        :node-key="componentOption.nodeKey"
        :accordion="componentOption.accordion"
        :empty-text="componentOption.emptyText"
        :show-checkbox="componentOption.showCheckbox"
        :check-strictly="componentOption.checkStrictly"
        :default-expand-all="componentOption.expandAll"
        :current-node-key="componentOption.currentNode"
        ...
        :check-on-click-node="componentOption.checkOnClickNode"
        :filter-node-method="filterNode"
        :indent="componentOption.indent || 8"
        :lazy="componentOption.lazy"
        :load="loadNode"
        :draggable="componentOption.draggable"
        :allow-drop="allowDrop"
        :allow-drag="allowDrag"
        :render-content="componentOption.slotScope?null:renderContent"
        :class="componentOption.treeClass"
      ...
      >
        <span
          v-if="componentOption.slotScope"
          slot-scope="{ node }"
          class="custom-tree-node tree-display"
          :class="[node.data.nodeDisabled?'is-disabled':'']"
          @mouseenter="showHideOptions(node,true)"
          @mouseleave="showHideOptions(node,false)"
        >
          <span class="tree-item-left">
            <span
              v-show="node.environment !== 'edit'"
              class="tree-item-label"
            >
              <el-tooltip
                :placement="componentOption.headSvg && componentOption.headSvg.placement"
              ...
              >
                ...
                <div class="tree-item-svg">
                  <svg-icon
                    v-if="node.data.headSvg || componentOption.headSvg"
                    :icon-class="labelSvg(node)"
                    :component-option="node.data.svgOption || componentOption.svgOption || {}"
                    class="tree-item-svg-svg"
                  />
                  <span v-if="node.data.headSvg && node.data.headSvg.badge" class="tree-item-svg-badge" />
                </div>
              </el-tooltip>
              <el-tooltip
                ref="tooltip"
                ...
                :disabled="detectWidth(node) && node.tootipDisabled"
                transition="fade-show"
              >
          ...
          <span class="tree-item-right">
            ...
            <el-tooltip v-if="node.data.rightSpan" :content="node.data.rightSpan" placement="top-start" :enterable="false">
              <span class="tree-item-rightSpan">
                {{ node.data.rightSpan }}
              </span>
            </el-tooltip>
            <span
              v-show="componentOption.totalNum &&
                (!(currentHoverNode.key === node.key) || !componentOption.iconOption) &&
                node.environment !== 'edit' &&
                !(node.data.loadingS || node.data.svg)"
              class="tree-item-num"
              :class="{'isIcon':componentOption.iconOption}"
            >
              {{ getTotalNum(node) }}
            </span>
            <span v-if="componentOption.iconOption" class="drop-icon" @click.stop>
              <transition name="fold">
                <div v-if="Object.keys(currentHoverNode).length >0 && currentHoverNode.key === node.key">
                  <el-button
                    v-if="node.environment !== 'edit' &&
                      permissibleConditions(componentOption.open,node) &&
                      !accessButtons({value:'openTree'})"
                    type="text"
                    icon="el-icon-plus"
                  ...
                  <el-button
                    v-if="node.environment !== 'edit' &&
                      ...
                      !accessButtons({value:'appendTree'})"
                    type="text"
                    icon="el-icon-plus"
                    @click="handleClickCommand({
                      command:'add',
                  ...
                  <el-button
                    v-if="node.environment === 'edit' &&
                      !accessButtons({value:'saveTree'})"
                    type="text"
                    icon="el-icon-check"
                    @click="saveNode(node,'click')"
                  />
                  <el-button
                    v-if="node.environment !== 'edit' &&
                      permissibleConditions(componentOption.delete,node) &&
                      !accessButtons({value:'deleteTree'})"
                    class="is-danger"
                    type="text"
                    icon="el-icon-delete"
                  ...
                  <el-button
                    v-if="permissibleConditions(null,node) &&
                      !accessButtons({value:'refreshTree'})"
                    type="text"
              ...
              <svg-icon
                v-show="node.data.loadingS || node.data.svg"
                v-loading-s="node.data.loadingS"
                :icon-class="node.data.svg || ''"
                class="drop-icon-svg"
              />
            </span>
            <point-drop-down
...
export default {
  ...
  methods: {
    ...
    editNode(node) {
      ...
      this.$set(node, 'environment', 'edit')
      this.$nextTick(() => {
    ...
    async detectWidth(node) {
      ...
      await this.$nextTick(() => {
        this.$set(node, 'tootipDisabled', that.$refs['span' + node.id][0].offsetWidth < that.$refs['div' + node.id][0].offsetWidth)
      })
    },
    reFreshNode(node) {
