<template>
    <div class="menu-item-container">
        ...
        <el-menu-item v-if="!componentOption.showAll && (!componentOption[prop] || componentOption[prop].length === 0)"
            :index="componentOption.value || resolvePath()" :disabled="checkIsMeta('disabled')"
            class="menu-item-padding">
            <el-badge
                :hidden="(componentOption.meta && componentOption.meta.badge && componentOption.badge.position === 'div') || (preconditions('value') == null)"
                :max="preconditions('max')" :type="preconditions('type')" :value="preconditions('value')"
                :is-dot="preconditions('isDot')" :class="preconditions('class')" class="menu-outer-badge">
                <div class="menu-item">
                    <i v-if="checkIsMeta('icon')" :class="checkIsMeta('icon')" />
                    <svg-icon v-if="checkIsMeta('svgIcon')" :class="[...(checkIsMeta('svgClass') || [])]"
                    ...
                    <el-badge
                        ...
                        :max="preconditions('max')" :type="preconditions('type')" :value="preconditions('value')"
                        :is-dot="preconditions('isDot')" :class="preconditions('class')" class="menu-inner-badge">
        ...
        <!-- el-submenu -->
        <el-sub-menu v-else :index="componentOption.value || resolvePath()" popper-append-to-body
            class="menu-item-padding" :popper-class="componentOption.popperClass || '' + ' ' + submenuClass" :class="[
            ...
            <template #title>
                <div class="submenu-menu-block">
                    <el-badge
                        ...
                        <div class="menu-item">
                            <i v-if="checkIsMeta('icon')" :class="checkIsMeta('icon')" />
                            <svg-icon v-if="checkIsMeta('svgIcon')" :class="[
                                'submenu-menu-title-svg',
                                ...(checkIsMeta('svgClass') || [])
                            ]" :icon-class="checkIsMeta('svgIcon')" />
                            <transition name="opacity-show">
                                <el-badge v-show="!aside.collapse"
                                    :hidden="(componentOption.meta && componentOption.meta.badge && componentOption.badge.position === 'span') || (preconditions('value') == null)"
                                    :max="preconditions('max')" :type="preconditions('type')"
                                    :value="preconditions('value')" :is-dot="preconditions('isDot')"
                                    :class="preconditions('class')" class="menu-inner-badge">
                                    {{ componentOption.label || componentOption.meta.title }}
                                </el-badge>
                            </transition>
                        </div>
            ...
            <div v-for="menuItem in isGroupMenu" :key="menuItem.path" class="submenu-menu-item">
                <el-menu-item-group v-if="menuItem.groupKey" :title="menuItem.groupKey">
                    <menu-item v-for="menuGroupItem in menuItem.groupData" :key="menuGroupItem.path"
                        :component-option="menuItem" :prop="prop" />
                </el-menu-item-group>

                <menu-item v-else :component-option="menuItem" :prop="prop" />
            </div>
        </el-sub-menu>
    </div>
</template>
