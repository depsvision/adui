<template>
  <div v-loading="componentOption.loading" class="form-list-container">
    <button-group
      v-if="componentOption.topButtons"
      :component-option="componentOption.topButtons"
    />
    <el-form
      ref="innerForm"
      :model="componentOption.data"
      ...
      :class="splitClass(componentOption)"
      @validate="validateForm"
    >
      <head-slot
        v-for="(headForm,headIndex) in formGroup"
        :key="headIndex"
        :component-option="headForm.head"
        :class="[componentOption.formGroup?'':'block-display']"
      >
        <el-form-item
          v-for="(form,formIndex) in headForm.form"
          v-show="!form.hide"
          :key="form.prop"
          :label="form.label"
          :prop="form.prop"
        ...
        >
          <span slot="label" class="form-item-label">
            ...
            <el-tooltip :disabled="!form.labelInfo" :content="form.labelInfo" placement="top" :style="form.labelInfoStyle">
              <svg-icon
          ...
          <div v-if="form.top" class="form-item-top" :style="form.top.topStyle">
            <component
              :is="form.top.type"
              ref="top"
              :component-option="dealSpanData(form.top)"
          ...
          <div v-if="form.back" class="form-item-back" :style="form.back.backStyle">
            <component
              :is="form.back.type"
              ref="back"
              :style="form.back.componentStyle"
          ...
          <span v-if="form.tip || (form.tipStyle && form.tipStyle.height)" class="form-item-label--tip" :style="form.tipStyle">{{ form.tip }}</span>

          <transition name="zoom-in-top">
            <div
              v-if="form.isInlineError && form.errorMessage"
          ...
          <div v-if="form.bottom" v-show="!form.hide" class="form-item-bottom-block">
            <div
              v-for="bottom in bottomGroup(form.bottom)"
              v-show="bottom.type"
              :key="bottom.prop"
              class="form-item-bottom"
              :style="form.bottom.bottomStyle"
            >
              <div class="form-item-bottom-content">
                ...
                <component
                  :is="bottom.type"
                  ref="bottom"
                  :component-option="dealSpanData(bottom)"
              ...
              <transition name="zoom-in-top">
                <div
                  v-if="bottom.isInlineError && bottom.errorMessage"
                  class="form-item--error form-item--inline_error"
                >
                  {{ bottom.errorMessage }}
                </div>
          ...
          <div v-if="form.imgList" class="form-item-img-list" :class="[form.imgList.disabled?'is-disabled':'']">
            <el-image
              v-for="img in componentOption.data[form.prop + 'ImgList']"
              :key="img[form.imgList.prop]"
              :src="img.url"
              :fit="form.imgList.fit"
          ...
          <div v-if="form.tool" class="form-item-tool">
            <div class="form-item-tool--tag">
              ...
              <el-tag v-for="tag in form.tool.tag" :key="tag.label" :style="tag.style" @click="handleClickFormTag(tag,form)">{{ tag.label }}</el-tag>
            </div>
          </div>

          <div
            v-if="form.isInnerError"
            slot="error"
            slot-scope="{ error }"
            class="form-item--error form-item--inner_error"
            :style="errorStyle(formIndex)"
          >
            {{ error }}
          </div>
        </el-form-item>
      ...
      <el-alert
        v-if="componentOption.alert"
        v-show="componentOption.alert.title || componentOption.alert.description"
        class="form-alert"
        :description="componentOption.alert.description"
        :type="componentOption.alert.type"
        :show-icon="componentOption.alert.showIcon"
        :closable="componentOption.alert.closable"
      >
        <span slot="title" class="form-alert-info">
          <span v-html="componentOption.alert.title" />
          <button-group
            v-if="componentOption.alert.buttons"
            class="form-alert-buttons"
  ...
  methods: {
    ...
    changeComponentValue(value, form) {
      ...
      if (form[form.prop + 'Path'] !== undefined) {
        this.$set(this.componentOption.data, form.prop + 'Path', form[form.prop + 'Path'])
      }
    ...
    validateForm(prop, status, error) {
      if (this.componentOption.form) {
        ...
        this.$set(this.componentOption.form[formIndex], 'errorMessage', !status ? error : '')
      } else {
        this.componentOption.formGroup.forEach(group => {
          ...
          if (form) {
            this.$set(form, 'errorMessage', !status ? error : '')
          }
        })
      }
    },
